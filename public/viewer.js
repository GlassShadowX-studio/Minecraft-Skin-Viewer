...
function buildBasePlanes(facesConfig, canvas, texH) {
    ...
    facesConfig.forEach(face => {
        ...
        // 核心修复 1：披�?Back �?UV 翻转，解决图案镜�?在里面的问题
        if (face.flipU) {
    ...
    facesConfig.forEach(face => {
        ...
        for (let i = 0; i < w; i++) {
            for (let j = 0; j < h; j++) {
                ...
                // 核心修复 3：移�?Gamma 校正，直接传�?sRGB 值，解决外覆层颜色过暗问�?
                const r = px[0] / 255;
                const g = px[1] / 255;
                const b = px[2] / 255;
...
function buildModel(canvas, isSlim, capeCanvas) {
    ...
    // 核心修复 2：手�?Pivot 向身体内部微�?0.1，消除共�?Z-fighting 导致的透明缝隙
    const PIVOTS = {
        head: [0, 28, 0],
        body: [0, 18, 0],
        rightLeg: [-2, 12, 0],
    ...
    if (capeCanvas) {
        ...
        if (capePart) {
            capePart.position.set(0, 6.5, -2.5);
